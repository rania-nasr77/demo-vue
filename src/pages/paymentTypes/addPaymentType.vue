<template>
  <div>
    <top-bar-card
      :newUrlName="`add-paymentType`"
      :submit="newPaymentType"
      :id="0"
      :editUrlName="`edit-paymentType`"
      :navigationUrl="`paymentTypeNavigation`"
      :model="`paymentType`"
      :mainModuel="`tools`"
      :MainCategory="`cards`"
      :pageName="`payment_type`"
      :operation="`add`"
      :listUrlName="`paymentType`"
    />
    <div
      class="card card-custom mb-9 mt-10"
      v-wow="{
        'animation-name': 'fadeInUp',
        'animation-duration': '.6s',
      }"
    >
      <div v-if="isEmpty">
        <errors :backErrors="backErrors" />
      </div>
      <div class="card-header flex-wrap border-0 pb-0 d-block">
        <form @submit.prevent="newPaymentType" id="paymentType">
          <div class="inf0-top-fixed">
            <h5 class="font-weight-bolder text-primary">
              {{ $t("payment_type") }}
            </h5>
            <div class="separator separator-solid my-3"></div>

            <div class="row">
              <div
                class="col-md-6 col-12 has-float-label position-relative"
                v-wow="{
                  'animation-name': 'fadeInUp',
                  'animation-duration': '1s',
                }"
              >
                <div class="form-group">
                  <input
                    type="numper"
                    class="form-control"
                    name="code"
                    :class="{
                      'is-invalid': $v.formData.code.$error,
                      'is-vaild': !$v.formData.code.$invalid,
                    }"
                    v-model="$v.formData.code.$model"
                    placeholder="Code"
                    ref="code"
                    id="code"
                    @keyup="chanagClass($event, `code`)"
                  />
                  <label for class="col-form-label">
                    {{ $t("code") }}
                    <span class="text-danger0">*</span>
                  </label>
                  <button
                    @click.prevent="clearValueInput(`code`)"
                    id="button_code"
                    class="close"
                    ref="button_code"
                  >
                    &times;
                  </button>
                </div>
              </div>
              <div class="col-12 d-flex p-0">
                <div
                  class="col-lg-6 has-float-label position-relative"
                  v-wow="{
                    'animation-name': 'fadeInUp',
                    'animation-duration': '1s',
                  }"
                >
                  <div class="form-group">
                    <input
                      type="text"
                      class="form-control w-100"
                      :class="{
                        'is-invalid': $v.formData.name_ar.$error,
                        'is-vaild': !$v.formData.name_ar.$invalid,
                      }"
                      :name="`name_ar`"
                      id="name_ar"
                      ref="name_ar"
                      v-model="$v.formData.name_ar.$model"
                      :placeholder="`Name`"
                      @keyup="chanagClass($event, `ar`)"
                    />

                    <label class="col-form-label">
                      {{ $t(`ar.name`) }}
                      <span class="text-danger0">*</span>
                    </label>
                    <span
                      class="text-danger"
                      v-if="backErrors[`name_ar`]"
                      :class="seenName"
                      >{{ backErrors[`name_ar`][0] }}</span
                    >

                    <button
                      @click.prevent="clearValueInput(`name_ar`, 'button_ar')"
                      id="button_ar"
                      class="close d-none"
                      ref="button_ar"
                    >
                      &times;
                    </button>
                  </div>
                </div>
                <div
                  class="col-lg-6 has-float-label position-relative"
                  v-wow="{
                    'animation-name': 'fadeInUp',
                    'animation-duration': '1s',
                  }"
                >
                  <div class="form-group">
                    <input
                      type="text"
                      class="form-control w-100"
                      :class="{
                        'is-invalid': $v.formData.name_en.$error,
                        'is-vaild': !$v.formData.name_en.$invalid,
                      }"
                      :name="`name_en`"
                      id="name_en"
                      ref="name_en"
                      v-model="$v.formData.name_en.$model"
                      :placeholder="`Name`"
                      @keyup="chanagClass($event, `en`)"
                    />

                    <label class="col-form-label">
                      {{ $t(`en.name`) }}
                      <span class="text-danger0">*</span>
                    </label>
                    <span
                      class="text-danger"
                      v-if="backErrors[`name_en`]"
                      :class="seenName"
                      >{{ backErrors[`name_en`][0] }}</span
                    >

                    <button
                      @click.prevent="clearValueInput(`name_en`, 'button_en')"
                      id="button_en"
                      class="close d-none"
                      ref="button_en"
                    >
                      &times;
                    </button>
                  </div>
                </div>
              </div>
              <div class="col-6 offset-1">
                <div class="form-group has-float-label">
                  <select
                    class="form-control"
                    name="temp_account_id"
                    :class="{
                      'is-invalid': $v.formData.temp_account_id.$error,
                    }"
                    v-model="$v.formData.temp_account_id.$model"
                  >
                    <option selected value></option>
                    <option value="1">Group.NO.1</option>
                    <option value="2">Group.NO.2</option>
                    <option value="3">Group.NO.3</option>
                    <option value="4">Group.NO.4</option>
                    <option value="5">Group.NO.5</option>
                  </select>

                  <label for class="col-form-label">
                    {{ $t("temp_account_id") }}
                  </label>

                  <span
                    class="text-danger"
                    v-if="backErrors[`temp_account_id`]"
                    :class="seenName"
                    >{{ backErrors[`temp_account_id`][0] }}</span
                  >
                </div>
              </div>

              <div class="col-6">
                <div class="form-group has-float-label">
                  <select
                    class="form-control"
                    name="tax_account_id"
                    :class="{
                      'is-invalid': $v.formData.tax_account_id.$error,
                    }"
                    v-model="$v.formData.tax_account_id.$model"
                  >
                    <option selected value></option>
                    <option value="1">Group.NO.1</option>
                    <option value="2">Group.NO.2</option>
                    <option value="3">Group.NO.3</option>
                    <option value="4">Group.NO.4</option>
                    <option value="5">Group.NO.5</option>
                  </select>

                  <label for class="col-form-label">
                    {{ $t("tax_account_id") }}
                  </label>

                  <span
                    class="text-danger"
                    v-if="backErrors[`tax_account_id`]"
                    :class="seenName"
                    >{{ backErrors[`tax_account_id`][0] }}</span
                  >
                </div>
              </div>
              <div
                class="col-md-6 col-12 has-float-label position-relative"
                v-wow="{
                  'animation-name': 'fadeInUp',
                  'animation-duration': '1s',
                }"
              >
                <div class="form-group">
                  <input
                    type="string"
                    class="form-control"
                    name="tax_percent"
                    :class="{
                      'is-invalid': $v.formData.tax_percent.$error,
                      'is-vaild': !$v.formData.tax_percent.$invalid,
                    }"
                    v-model="$v.formData.tax_percent.$model"
                    placeholder="tax_percent"
                    ref="tax_percent"
                    id="tax_percent"
                    @keyup="chanagClass($event, `tax_percent`)"
                  />
                  <label for class="col-form-label">
                    {{ $t("tax_percent") }}
                  </label>
                  <button
                    @click.prevent="clearValueInput(`tax_percent`)"
                    id="button_tax_percent"
                    class="close"
                    ref="button_tax_percent"
                  >
                    &times;
                  </button>
                </div>
              </div>
              <div class="col-6">
                <div class="form-group has-float-label">
                  <select
                    class="form-control"
                    name="fees_account_id"
                    :class="{
                      'is-invalid': $v.formData.fees_account_id.$error,
                    }"
                    v-model="$v.formData.fees_account_id.$model"
                  >
                    <option selected value></option>
                    <option value="1">Group.NO.1</option>
                    <option value="2">Group.NO.2</option>
                    <option value="3">Group.NO.3</option>
                    <option value="4">Group.NO.4</option>
                    <option value="5">Group.NO.5</option>
                  </select>

                  <label for class="col-form-label">
                    {{ $t("fees_account_id") }}
                  </label>

                  <span
                    class="text-danger"
                    v-if="backErrors[`fees_account_id`]"
                    :class="seenName"
                    >{{ backErrors[`fees_account_id`][0] }}</span
                  >
                </div>
              </div>

              <div
                class="col-md-6 col-12 has-float-label position-relative"
                v-wow="{
                  'animation-name': 'fadeInUp',
                  'animation-duration': '1s',
                }"
              >
                <div class="form-group">
                  <input
                    type="string"
                    class="form-control"
                    name="max_fees_value"
                    :class="{
                      'is-invalid': $v.formData.max_fees_value.$error,
                      'is-vaild': !$v.formData.max_fees_value.$invalid,
                    }"
                    v-model="$v.formData.max_fees_value.$model"
                    placeholder="max_fees_value"
                    ref="max_fees_value"
                    id="max_fees_value"
                    @keyup="chanagClass($event, `max_fees_value`)"
                  />
                  <label for class="col-form-label">
                    {{ $t("max_fees_value") }}
                  </label>
                  <button
                    @click.prevent="clearValueInput(`max_fees_value`)"
                    id="button_max_fees_value"
                    class="close"
                    ref="button_max_fees_value"
                  >
                    &times;
                  </button>
                </div>
              </div>

              <div
                class="col-6"
                v-wow="{
                  'animation-name': 'fadeInUp',
                  'animation-duration': '1s',
                }"
              >
                <fieldset class="d-flex">
                  <legend>{{ $t(`fees_type`) }}</legend>
                  <div class="col-6">
                    <div class="row">
                      <label
                        class="
                          radio radio-outline radio-outline-2x radio-primary
                          col-6
                          m-0
                          py-2
                        "
                      >
                        <input
                          type="radio"
                          :name="`fees_type`"
                          v-model="$v.formData.fees_type.$model"
                          :value="1"
                        />
                        <span></span>
                        {{ $t("percent") }}
                      </label>

                      <label
                        class="
                          radio radio-outline radio-outline-2x radio-primary
                          col-6
                          m-0
                        "
                      >
                        <input
                          type="radio"
                          :name="`fees_type`"
                          v-model="$v.formData.fees_type.$model"
                          :value="0"
                        />
                        <span></span>
                        {{ $t("value") }}
                      </label>
                    </div>
                  </div>
                  <div
                    class="col-md-6 col-12 has-float-label position-relative"
                    v-wow="{
                      'animation-name': 'fadeInUp',
                      'animation-duration': '1s',
                    }"
                  >
                    <div class="form-group">
                      <input
                        type="numper"
                        class="form-control"
                        name="fees_value"
                        :class="{
                          'is-invalid': $v.formData.fees_value.$error,
                          'is-vaild': !$v.formData.fees_value.$invalid,
                        }"
                        v-model="$v.formData.fees_value.$model"
                        placeholder="fees_value"
                        ref="fees_value"
                        id="fees_value"
                        @keyup="chanagClass($event, `fees_value`)"
                      />
                      <label for class="col-form-label">
                        {{ $t("fees_value") }}
                      </label>
                      <button
                        @click.prevent="clearValueInput(`fees_value`)"
                        id="button_fees_value"
                        class="close"
                        ref="button_fees_value"
                      >
                        &times;
                      </button>
                    </div>
                  </div>
                </fieldset>
              </div>

              <div
                class="col-6"
                v-wow="{
                  'animation-name': 'fadeInUp',
                  'animation-duration': '1s',
                }"
              >
                <div class="d-flex">
                  <active-toggle :formData="formData" class="col p-0" />
                </div>
              </div>
            </div>
          </div>
        </form>
      </div>
    </div>
  </div>
</template>

<script>
import { useVuelidate } from "@vuelidate/core";
import { required, requiredIf, requiredUnless } from "@vuelidate/validators";
import { UAnimateContainer, UAnimate } from "vue-wow";
export default {
  name: "add-paymentType",
  components: { UAnimateContainer, UAnimate },

  props: {
    backErrors: {
      type: Object,
      default: () => ({}),
    },
  },
  data() {
    return {
      formData: {
        max_fees_value: "",
        tax_percent: "",
        fees_value: "",
        temp_account_id: "",
        fees_account_id: "",
        tax_account_id: "",
        code: "",
        is_active: true,
        name_ar: "",
        name_en: "",
        dtype: 2,
        fees_type: "1",
        days_count: "",
      },
      isEmpty: false,
      is_show: "",
      backErrors: {},
    };
  },
  validations() {
    return {
      formData: {
        name_ar: {
          requiredIfNameAr: requiredIf(this.formData.name_en == ""),
        },
        name_en: {
          requiredIfNameEn: requiredIf(this.formData.name_ar == ""),
        },
        code: { required },
        fees_value: {},
        max_fees_value: {},
        tax_percent: {},
        temp_account_id: {},
        fees_account_id: {},
        tax_account_id: {},
        fees_type: {},
        // days_count: {
        //   requiredIfFollowRenewal: requiredIf(
        //     this.formData.follow_renewal == true
        //   ),
        // },
      },
    };
  },
  /////
  setup: () => ({ $v: useVuelidate() }),
  mounted() {
    this.getCode();
  },
  methods: {
    async newPaymentType() {
      ////

      this.$v.$touch();
      if (this.$v.$invalid) {
        return; //
      } else {
        try {
          this.formData.name_ar = this.formData.name_ar
            ? this.formData.name_ar
            : this.formData.name_en;
          this.formData.name_en = this.formData.name_en
            ? this.formData.name_en
            : this.formData.name_ar;

          await this.$store.dispatch("storePaymentType", {
            data: this.formData,
          });
        } catch (errors) {
          this.backErrors = errors;
          this.isEmpty = true;
        }
      }
    },

    /**
     *
     * to clear any data in any button
     */
    clearValueInput(inputId, buttonId) {
      this.formData[`${inputId}`] = "";
      this.$refs[`${buttonId}`].classList.add("d-none");
    },
    /**
     *
     * to add and remove class d-none
     */
    chanagClass(e, id) {
      if (e.target.value !== "") {
        this.$refs[`button_${id}`].classList.remove(`d-none`);
      } else {
        this.$refs[`button_${id}`].classList.add(`d-none`);
      }
    },
    valinput(e) {
      if (e.target.value == "") {
        this.addclass = "is-invalid";
        this.seenName = "";
      } else {
        this.seenName = "d-none";
      }
    },
    async getCode() {
      const code = await this.$store.dispatch("getPaymentTypeCode");

      this.formData.code = code.data.data;
    },
  },
};
</script>

<style>
</style>