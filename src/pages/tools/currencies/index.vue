...
export default {
  ...
  data() {
    return {
      ...
      url: `http://127.0.0.1:8000/api/currencies/export`,
      lang: "ar",
      columns: [
        {
          label: this.$t("id"),
  ...
  methods: {
    async getResults() {
      const currencyUrl = { URL: "currencies", all: 1 };
      const currencies = await this.$store.dispatch(
        "fetchCurrencies",
        currencyUrl
      );
      this.rows = this.$store.getters.getCurrencies.data;
    },
    async deleteCurrency(id) {
      try {
        ...
        if (result.value) {
          ...
          await this.$store.dispatch("deleteCurrency", URL);
