<template>
  <div>
    <top-bar-card
      ...
      :submit="updateCarStatus"
      :id="formData.id"
      :editUrlName="`edit-CarStatus`"
      :navigationUrl="`CarStatusNavigation`"
      :model="`CarStatus`"
      :listUrlName="`CarStatus`"
      :mainModuel="`tools`"
      :MainCategory="`cards`"
      :pageName="`CarStatus`"
      :operation="`edit`"
    ...
    <div class="card card-custom mb-9 mt-10">
      ...
      <div class="card-header flex-wrap border-0 pb-0 d-block">
        <form @submit.prevent="updateCarStatus" id="CarStatus">
...
export default {
    ...
    return {
      formData: {
        name_ar: {
          ...
          requiredIfFuction: requiredUnless(this.validator),
        },
        name_en: {
  ...
  methods: {
    async updateCarStatus() {
      ...
      if (this.$v.$invalid) {
        ...
        try {
          ...
          await this.$store.dispatch("updateCarStatus", params);
        } catch (errors) {
    ...
    async getCarStatus() {
      const getCarStatus = await this.$store.dispatch("getCarStatus", {
        Id: this.$route.params.id,
      });
      this.formData = getCarStatus.data.data;
    },
    validatorName() {
