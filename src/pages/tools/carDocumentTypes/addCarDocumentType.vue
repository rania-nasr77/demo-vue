<template>
  <div>
    <top-bar-card
      ...
      :submit="newCarDocumentType"
      :id="0"
      :editUrlName="`edit-carDocumentType`"
      :navigationUrl="`carNavigation`"
    ...
    >
      ...
      <div class="card-header flex-wrap border-0 pb-0 d-block">
        <form @submit.prevent="newCarDocumentType" id="carDocumentType">
          <div class="inf0-top-fixed">
            ...
            <div class="row align-items-end">
              ...
              <active-toggle
                ...
                :formData="formData"
                :name="`follow_renewal`"
                :label="$t(`follow_renewal`)"
              />


              

              <div class="has-float-label col position-relative">
                <div class="form-group" v-if="formData.follow_renewal">
                  <input
                    ...
                    class="form-control"
                    :class="{
                      'is-invalid': $v.formData.days_count.$error,
                      'is-vaild': !$v.formData.days_count.$invalid,
                    }"
                    @keyup="chanagClass($event, `days_count`)"
                  />
                  <label for class="col-form-label">
                    {{ $t("days_count") }}
                    <span class="text-danger0">*</span>
                  ...
                  <button
                    @click.prevent="
                      clearValueInput(`days_count`, 'button_days_count')
                    "
                    id="button_days_count"
                    class="close d-none"
                    ref="button_days_count"
                  >&times;</button>
                </div>
              </div>
              <!-- </div> -->
...
export default {
  ...
  methods: {
    async newCarDocumentType() {
      ...
      if (this.$v.$invalid) {
        ...
        try {
          ...
          await this.$store.dispatch("storeCarDocumentType", {
            data: this.formData
          });
        } catch (errors) {
    ...
    async getCode() {
      const code = await this.$store.dispatch("getCarDocumentTypeCode");
