<template>
  <div>
    <top-bar-card
      :newUrlName="`edit-districts`"
      :submit="updateDistrict"
      :id="formData.id"
      :editUrlName="`edit-districts`"
      :navigationUrl="`districts`"
      :model="`district`"
      :mainModuel="`tools`"
      :MainCategory="`cards`"
      :pageName="`districts`"
      :operation="`edit`"
      :listUrlName="`districts`"
    />
    <div class="card card-custom mb-9 mt-10">
      ...
      <div class="card-header flex-wrap border-0 pb-0 d-block">
        <form @submit.prevent="updateDistrict" id="district">
...
export default {
  ...
  methods: {
    async updateDistrict() {
      ...
      if (this.$v.$invalid) {
        ...
        try {
          ...
          await this.$store.dispatch("updateDistrict", params);
        } catch (errors) {
    ...
    async getDistrict() {
      const getDistrict = await this.$store.dispatch("getDistrict", {
        Id: this.$route.params.id,
      });
