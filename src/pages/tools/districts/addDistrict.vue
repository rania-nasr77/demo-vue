<template>
  <div>
    <top-bar-card
      :newUrlName="`add-districts`"
      :submit="newDistrict"
      :id="0"
      :editUrlName="`edit-districts`"
      :navigationUrl="`districtNavigation`"
      :model="`district`"
      :mainModuel="`tools`"
      :MainCategory="`cards`"
      :pageName="`districts`"
    ...
    >
      ...
      <div class="card-header flex-wrap border-0 pb-0 d-block">
        <form @submit.prevent="newDistrict" id="district">
...
export default {
  ...
  methods: {
    async newDistrict() {
      ...
      if (this.$v.$invalid) {
        ...
        try {
          ...
          await this.$store.dispatch("storeDistrict", {
            data: this.formData,
          });
        } catch (errors) {
    ...
    async getCode() {
      const code = await this.$store.dispatch("getDistrictCode");
