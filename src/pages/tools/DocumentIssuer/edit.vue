<template>
  <div>
    <top-bar-card
      :newUrlName="`add_DocumentIssuer`"
      :listUrlName="`DocumentIssuer`"
      :submit="updateDocumentIssuer"
      :id="formData.id"
      :editUrlName="`edit-DocumentIssuer`"
      :navigationUrl="`DocumentIssuerNavigation`"
      :model="`DocumentIssuer`"
      :mainModuel="`tools`"
      :MainCategory="`cards`"
      :pageName="`DocumentIssuer`"
      :operation="`edit`"
    />
    <div class="card card-custom mb-9 mt-10">
      ...
      <div class="card-header flex-wrap border-0 pb-0 d-block">
        <form @submit.prevent="updateDocumentIssuer" id="DocumentIssuer">
...
export default {
  ...
  methods: {
    async updateDocumentIssuer() {
      ...
      if (this.$v.$invalid) {
        ...
        try {
          ...
          await this.$store.dispatch("updateDocumentIssuer", params);
        } catch (errors) {
    ...
    async getDocumentIssuer() {
      const getDistrict = await this.$store.dispatch("getDocumentIssuer", {
        Id: this.$route.params.id,
      });
      this.formData = getDistrict.data.data;
    },
