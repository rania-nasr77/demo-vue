      ...
      >
        <template slot="table-row" slot-scope="props">
          <span v-if="props.column.field == 'actions'">
            ...
            <edit-action
              :id="props.row.id"
              editUrlName="edit_DocumentIssuer"
            ></edit-action>
...
export default {
  ...
  data() {
    return {
      ...
      url: `http://127.0.0.1:8000/api/DocumentIssuer/export`,
      lang: "ar",
      columns: [
        {
          label: this.$t("code"),
  ...
  computed: {
    DocumentIssuer() {
      return this.$store.getters.getDocumentIssuer;
    },
    visibleFields() {
  ...
  methods: {
    async getResults() {
      const DocumentIssuerUrl = { URL: "document-issuer" };
      const DocumentIssuer = await this.$store.dispatch(
        "fetchDocumentIssuer",
        DocumentIssuerUrl
      );
      this.rows = this.$store.getters.getDocumentIssuer.data;
    },
      ...
      if (result.value) {
        await this.$store.dispatch("deleteDocumentIssuer", id);
