<template>
  <div>
    <top-bar-card
      ...
      :newUrlName="`edit-trailerWheels`"
      :submit="updatetrailerWheel"
      :id="formData.id"
      :editUrlName="`edit-trailerWheels`"
      :navigationUrl="`trailerWheelNavigation`"
      :model="`trailerWheel`"
      :listUrlName="`Trailer Wheel`"
      :mainModuel="`tools`"
      :MainCategory="`cards`"
      :pageName="`Trailer Wheel`"
      :operation="`edit`"
    ...
    <div class="card card-custom mb-9 mt-10">
      ...
      <div class="card-header flex-wrap border-0 pb-0 d-block">
        <form @submit.prevent="updatetrailerWheel" id="trailerWheels">
...
export default {
  ...
  mounted() {
    this.gettrailerWheel();
  },

  methods: {
    async gettrailerWheel() {
      const gettrailerWheel = await this.$store.dispatch("gettrailerWheel", {
        Id: this.$route.params.id,
      });
      this.formData = gettrailerWheel.data.data;
    },

    async updatetrailerWheel() {
      this.$v.$touch();
      if (this.$v.$invalid) {
        return;
      } else {
        try {
          ...
          await this.$store.dispatch("updatetrailerWheel", params);
        } catch (errors) {
          console.log(errors);
