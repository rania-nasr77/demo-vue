<template>
  <div>
    <top-bar-card
      :newUrlName="`edit-carTypes`"
      :submit="updatecarType"
      :id="formData.id"
      :editUrlName="`edit-carTypes`"
      :navigationUrl="`vehicle-types`"
      :model="`carType`"
      :listUrlName="`carTypes`"
      :mainModuel="`tools`"
      :MainCategory="`cards`"
      :pageName="`Car Type`"
      :operation="`edit`"
...
export default {
  ...
  methods: {
    async getcarType() {
      const getcarType = await this.$store.dispatch("getcarType", {
        Id: this.$route.params.id,
      });
      this.formData = getcarType.data.data;
    },
    async updatecarType() {
      this.$v.$touch();
      if (this.$v.$invalid) {
        ...
        try {
          ...
          await this.$store.dispatch("updatecarType", params);
        } catch (errors) {
