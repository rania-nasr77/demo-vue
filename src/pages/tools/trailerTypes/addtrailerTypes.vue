<template>
  <div>
    <top-bar-card
      :newUrlName="`add-trailerTypes`"
      :submit="newtrailerTypes"
      :id="0"
      :editUrlName="`edit-trailerTypes`"
      :navigationUrl="`trailerTypeNavigation`"
      :model="`trailerType`"
      :listUrlName="`trailer Types`"
      :mainModuel="`tools`"
      :MainCategory="`cards`"
    ...
    >
      ...
      <div class="card-header flex-wrap border-0 pb-0 d-block">
        <form @submit.prevent="newtrailerTypes" id="trailerType">
...
export default {
  ...
  methods: {
    async newtrailerTypes() {
      ...
      if (this.$v.$invalid) {
        ...
        try {
          ...
          await this.$store.dispatch("storetrailerTypes", {
            data: this.formData,
          });
        } catch (errors) {
    ...
    async getCode() {
      await $axios
        .get("vehicle-types/code?vtype=2")
        .then((response) => {
