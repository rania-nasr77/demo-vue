<template>
  <div>
    <top-bar-card
      :newUrlName="`edit-trailerTypes`"
      :submit="updatetrailerType"
      :id="formData.id"
      :editUrlName="`edit-trailerTypes`"
      :navigationUrl="`trailerTypeNavigation`"
      :model="`trailerType`"
      :listUrlName="`trailerTypes`"
      :mainModuel="`tools`"
      :MainCategory="`cards`"
      :pageName="`Trailer Types`"
      :operation="`edit`"
    ...
    <div class="card card-custom mb-9 mt-10">
      ...
      <div class="card-header flex-wrap border-0 pb-0 d-block">
        <form @submit.prevent="updatetrailerType" id="trailerType">
...
export default {
  ...
  methods: {
    async gettrailerType() {
      const gettrailerType = await this.$store.dispatch("gettrailerType", {
        Id: this.$route.params.id,
      });
      this.formData = gettrailerType.data.data;
    },

    async updatetrailerType() {
      this.$v.$touch();
      if (this.$v.$invalid) {
        return;
      } else {
        try {
          ...
          await this.$store.dispatch("updatetrailerType", params);
        } catch (errors) {
