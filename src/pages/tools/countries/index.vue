...
export default {
  ...
  data() {
    return {
      ...
      url: `http://127.0.0.1:8000/api/countries/export`,
      lang: "ar",
      columns: [
        {
          label: this.$t("id"),
  ...
  methods: {
    async getResults() {
      ...
      await this.$store.dispatch("fetchCountries", countryUrl);
      this.rows = this.$store.getters.getCountries.data;
    },
    async Delete(id) {
      ...
      if (result.value) {
        ...
        await this.$store.dispatch("deleteCountry", URL);
