      ...
      <div class="card-header flex-wrap border-0 pb-0 d-block">
        <form @submit.prevent="updateTax" id="currentForm">
          <div class="inf0-top-fixed">
            ...
            <div>
              <div v-if="CountriesTax.length > 0">
                <editCountires :errors="errors" :CountriesTax="CountriesTax" />
              </div>
              <div v-else>
                <countries :errors="errors" />
              </div>
            </div>
          </div>
        </form>
...
import EditCountires from "./EditCountires.vue";

export default {
  name: "add-taxes",
  components: { countries, EditCountires },
  ...
  methods: {
    async updateTax() {
      try {
        ...
        const tax = {
          URL: "/taxes/" + this.$route.params.id,
          data: formData,
        };
        await this.$store.dispatch("updateTax", tax);
        document.location.href = "taxes";
      } catch (errors) {
        this.errors = errors;
      }
    },
    ...
    async newTax() {
      try {
        var form = document.getElementById("currentForm");
        var formData = new FormData(form);
        const tax = { URL: "taxes", data: formData };
    ...
    getTax() {
      $axios
        .get("/taxes/" + this.$route.params.id)
        .then((response) => {
