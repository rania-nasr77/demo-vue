<template>
  <div>
    <top-bar-card
      :newUrlName="`add-units`"
      :submit="newUnit"
      :model="`unit`"
      :id="0"
      :editUrlName="`edit-units`"
      :navigationUrl="`unitsNavigation`"
      :listUrlName="`units`"
      :mainModuel="`tools`"
      :MainCategory="`cards`"
      :pageName="$t(`units`)"
    ...
    >
      ...
      <div class="card-header flex-wrap border-0 pb-0 d-block">
        <form @submit.prevent="newUnit" id="units">
                ...
                >
                  <div class="form-group">
                    <input
                      ...
                      ref="name_en"
                      :class="{
                        'is-invalid': $v.formData.name_en.$error || backErrors[`code`]
                      }"
...
export default {
  ...
  methods: {
    async newUnit() {
      ...
      if (this.$v.$invalid) {
        ...
        try {
          ...
          await this.$store.dispatch("storeUnit", {
            data: this.formData,
          });
        } catch (errors) {
    ...
    async getCode() {
        const code = await this.$store.dispatch("getUnitCode");
