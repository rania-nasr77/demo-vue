<template>
  <div>
    <top-bar-card
      :newUrlName="`edit-carClassifications`"
      :submit="updateCarClassification"
      :id="formData.id"
    ...
    <div class="card card-custom mb-9 mt-10">
      ...
      <div class="card-header flex-wrap border-0 pb-0 d-block">
        <form @submit.prevent="updatecarType" id="carType">
...
export default {
  ...
  methods: {
     ...
     * get data from db
     */
    async getCarClassification() {
      const getCarClassification = await this.$store.dispatch(
        "getCarClassification",
        {
          Id: this.$route.params.id,
        }
      );
      this.formData = getCarClassification.data.data;
    },
    /**
     * updata data
     */
    async updateCarClassification() {
      this.$v.$touch();
      if (this.$v.$invalid) {
        return;
      } else {
        try {
          ...
          await this.$store.dispatch("updateCarClassification", params);
        } catch (errors) {
