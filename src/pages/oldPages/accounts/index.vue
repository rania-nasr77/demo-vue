<template>
  <div>
    <top-bar-card :newUrlName="`add-accounts`" />
...
export default {
  ...
  data() {
    return {
      ...
      url: `http://127.0.0.1:8000/api/accounts/export`,
      lang: "ar",
      columns: [
        {
          label: this.$t("id"),
  ...
  methods: {
    async getResults() {
      const accountUrl = { URL: "accounts", all: 1 };
      const accounts = await this.$store.dispatch("fetchAccounts", accountUrl);
      this.rows = this.$store.getters.getAccounts.data;
    },
    ...
    async Delete(id) {
      ...
      if (result.value) {
        ...
        await this.$store.dispatch("deleteAccount", URL);
