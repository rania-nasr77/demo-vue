<template>
  <div>
    <top-bar-card :newUrlName="`add-discounts`" />
      ...
      >
        <template slot="table-row" slot-scope="props">
          <span v-if="props.column.field == 'actions'">
            <show-action
              :id="props.row.id"
              showUrlName="show-discounts"
            ></show-action>
            <edit-action
              :id="props.row.id"
              editUrlName="edit-discounts"
            ></edit-action>
            <delete-action
              :id="props.row.id"
...
export default {
  ...
  data() {
    return {
      ...
      url: `http://127.0.0.1:8000/api/discounts/export`,
      lang: "ar",
      columns: [
        {
          label: this.$t("id"),
  ...
  methods: {
    async getResults() {
      const discountUrl = { URL: "discounts", all: 1 };
      const discounts = await this.$store.dispatch(
        "fetchDiscounts",
        discountUrl
      );
      this.rows = this.$store.getters.getDiscounts.data;
    },

    async deletediscounts(discount_id) {
      try {
        const result = await this.$SwalChecks(
          "You won't be able to revert this!"
        );
        if (result.value) {
          const URL = "discounts/" + discount_id;
          await $axios.delete(URL).then((response) => {
            this.rows = this.rows.filter((discount) => {
