      ...
      >
        <template slot="table-row" slot-scope="props">
          <span v-if="props.column.field == 'actions'">
            <show-action
              :id="props.row.id"
              showUrlName="show-classifications"
            ></show-action>
            <edit-action
              :id="props.row.id"
...
export default {
  ...
  data() {
    return {
      ...
      url: `http://127.0.0.1:8000/api/classifications/export`,
      lang: "ar",
      columns: [
        {
          label: this.$t("id"),
  ...
  methods: {
    async getResults() {
      const classificationUrl = { URL: "classifications", all: 1 };
      const classifications = await this.$store.dispatch(
        "fetchClassifications",
        classificationUrl
      );
      this.rows = this.$store.getters.getClassifications.data;
    },
    async deleteClassification(id) {
      try {
        ...
        if (result.value) {
          ...
          await this.$store.dispatch("deleteClassification", URL);
