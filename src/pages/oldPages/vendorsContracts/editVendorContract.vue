<template>
  <div>
    <top-bar-card
      :newUrlName="`edit-vendorsContracts`"
      :submit="updatevendorsContract"
      @onChangeGrid="changeGrid"
      :id="vendorsContract.id"
    ...
    <div class="card card-custom mb-9 mt-10">
      <div class="card-header flex-wrap border-0 pb-0 d-block">
        <form @submit.prevent="updatevendorsContract" id="currentForm">
                ...
                <lang-tabs :languages="languages">
                  <template v-slot:input="{ language }">
                    <editInputs
                      :language="language"
                      :errors="errors"
                      :vendorsContract="vendorsContract"
                    />
                  </template>
                </lang-tabs>
              </div>
                ...
                <div class="has-float-label">
                  <div class="form-group">
                    <input
                      ...
                      name="days"
                      placeholder="Days"
                      :value="vendorsContract.days"
                    />
                    <label for="" class="col-form-label">Days</label>
                  </div>
                ...
                <div class="has-float-label">
                  <div class="form-group m-0">
                    <input
                      ...
                      :value="vendorsContract.expiry_date"
                    />
                    <label for="" class="col-form-label">{{
                      $t("expiry_date")
                    }}</label>
                          ...
                          <div class="col has-float-label">
                            <div class="form-group">
                              <input
                                ...
                                :value="vendorsContract.account_value"
                              />
                              <label for="" class="col-form-label">{{
                            ...
                            <div class="form-group has-float-label">
                              ...
                              >
                                ...
                                <option
                                  value="first account"
                                  :selected="
                                    vendorsContract.discount_account ==
                                    'first account'
                                  "
                                >
                                  {{ $t("first_account") }}
                            ...
                            <div class="form-group has-float-label">
                              ...
                              >
                                ...
                                <option
                                  value="first account"
                                  :selected="
                                    vendorsContract.vendor_account ==
                                    'first account'
                                  "
                                >
                                  {{ $t("first_account") }}
                            ...
                            <div class="form-group has-float-label">
                              <select class="form-control" name="account">
                                ...
                                <option
                                  value="first account"
                                  :selected="
                                    vendorsContract.account == 'first account'
                                  "
                                >
                                  {{ $t("first_account") }}
                                </option>
                                <option
                                  value="second account"
                                  :selected="
                                    vendorsContract.account == 'second account'
                                  "
                                >
                                  {{ $t("second_account") }}
                                </option>
...
export default {
  ...
  created() {
    this.vendorsContractId = this.$route.params.id;
    this.getVendorsContract();
  },

  methods: {
    async updatevendorsContract() {
      try {
        console.log("updated");
        ...
        const vendorsContract = {
          URL: "/vendorsContracts/" + this.$route.params.id,
          data: formData,
        };
        await this.$store.dispatch("updatevendorsContract", vendorsContract);
    ...
    getVendorsContract() {
      $axios
        .get("/vendorsContracts/" + this.vendorsContractId)
        .then((response) => {
          console.log(response.data.data.id);
          this.vendorsContract = response.data.data;
        })
        .catch((error) => {});
    },
