<template>
  <div>
    <top-bar-card :newUrlName="`add_Tabulations`" />
...
export default {
  ...
  data() {
    return {
      ...
      url: `http://127.0.0.1:8000/api/tabulations/export`,
      lang: "ar",
      columns: [
        {
          label: this.$t("id"),
  ...
  methods: {
    async getResults() {
      ...
      await this.$store.dispatch("fetchtabulations", tabulationUrl);
      this.rows = this.$store.getters.gettabulations.data;
    },
    ...
    async Delete(id) {
      try {
        ...
        if (result.value) {
          const URL = "tabulations/" + id;
          await this.$store.dispatch("deleteSubCategory", URL);
          this.rows = this.rows.filter((tabulation) => {
            return tabulation.id != id;
          });
        }
      } catch (errors) {
        this.errors = errors;
