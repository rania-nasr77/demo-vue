<template>
  <div>
    <top-bar-card :newUrlName="`add_Customnotification`" />
      ...
      >
        <template slot="table-row" slot-scope="props">
          <span v-if="props.column.field == 'actions'">
            <show-action
              :id="props.row.id"
              showUrlName="show-Customnotification"
            ></show-action>
            <edit-action
              :id="props.row.id"
...
export default {
  ...
  data() {
    return {
      ...
      url: `http://127.0.0.1:8000/api/Customnotification/export`,
      lang: "ar",
      columns: [
        {
          label: this.$t("id"),
  ...
  methods: {
    async getResults() {
      const customnotificationUrl = { URL: "custom-notifications", all: 1 };
      const customnotifications = await this.$store.dispatch(
        "fetchCustomnotifications",
        customnotificationUrl
      );

      this.laravelData = customnotifications;
      this.rows = this.$store.getters.getCustomnotifications.data;
    },
    async Delete(id) {
      try {
        ...
        if (result.value) {
          const URL = "customnotifications/" + id;
          await this.$store.dispatch("deleteCustomnotification", URL);
