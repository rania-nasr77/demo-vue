<template>
  <div>
    <top-bar-card :newUrlName="`add-cards`" />
...
export default {
  ...
  data() {
    return {
      ...
      url: `http://127.0.0.1:8000/api/cards/export`,
      lang: "ar",
      columns: [
        {
          label: this.$t("id"),
  ...
  methods: {
    async getResults() {
      const cardUrl = { URL: "cards", all: 1 };
      const cards = await this.$store.dispatch("fetchCards", cardUrl);
      this.rows = this.$store.getters.getCards.data;
    },
    async Delete(id) {
      ...
      if (result.value) {
        const URL = "cards/" + id;
        await this.$store.dispatch("deleteCard", URL);
