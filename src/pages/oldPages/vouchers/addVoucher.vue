<template>
  <div>
    <top-bar-card
      :newUrlName="`add-vouchers`"
      :submit="newVoucher"
      :id="0"
      :editUrlName="`edit-vouchers`"
      :navigationUrl="`voucherNavigation`"
      :model="`voucher`"
    />

    <div
      class="card card-custom mb-9 mt-10"
      v-wow="{
        'animation-name': 'fadeInUp',
        'animation-duration': '.6s',
      }"
    >
      <div v-if="isEmpty">
        <errors :errors="errors" />
      </div>

      <div class="card-header flex-wrap border-0 pb-0 d-block">
        <form @submit.prevent="newVoucher" id="currentForm">
          <div class="inf0-top-fixed">
            <h5 class="font-weight-bolder text-primary">
              {{ $t("general_info") }}
            </h5>
            <div class="separator separator-solid my-3"></div>
            <div class="row align-items-end">
              <div
                class="col-6"
                v-wow="{
                  'animation-name': 'fadeInUp',
                  'animation-duration': '1s',
                }"
              >
                <lang-tabs :languages="languages">
                  <template v-slot:input="{ language }">
                    <inputs :language="language" :errors="errors" />
                  </template>
                </lang-tabs>
              </div>
              <div
                class="col-md-6 col-12 mt-7"
                v-wow="{
                  'animation-name': 'fadeInUp',
                  'animation-duration': '1s',
                }"
              >
                <div class="form-group m-0 has-float-label">
                  <select id="" class="form-control" name="type">
                    <option selected value=""></option>
                    <option value="in">{{ $t("In") }}</option>
                    <option value="out">{{ $t("out") }}</option>
                  </select>
                  <label for="" class="col-form-label">{{ $t("type") }}</label>
                </div>
              </div>
            </div>
          </div>
        </form>
      </div>
    </div>
  </div>
</template>

<script>
import LangTabs from "@/components/form/LangTabs.vue";
import Errors from "@/components/layouts/errors.vue";
import Inputs from "./Inputs.vue";
import { UAnimateContainer, UAnimate } from "vue-wow";
export default {
  name: "add-vouchers",
  components: { LangTabs, Inputs, Errors, UAnimateContainer, UAnimate },

  data() {
    return {
      voucher: {},
      languages: window.$languages,
      errors: {},
      isEmpty: false,
    };
  },

  methods: {
    async newVoucher() {
      try {
        var form = document.getElementById("currentForm");
        var formData = new FormData(form);
        const voucher = { URL: "vouchers", data: formData };
        await this.$store.dispatch("storeVouchers", voucher);
        document.location.href = "vouchers";
      } catch (errors) {
        this.errors = errors;
        this.isEmpty = true;
      }
    },
  },
};
</script>

<style></style>
