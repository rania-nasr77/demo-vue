<template>
  <div>
    <top-bar-card :newUrlName="`add-products`" />
      ...
      >
        <template slot="table-row" slot-scope="props">
          <span v-if="props.column.field == 'actions'">
            <show-action
              :id="props.row.id"
              showUrlName="show-products"
            ></show-action>
            <edit-action
              :id="props.row.id"
              :editUrlName="`products.edit`"
            ></edit-action>
...
export default {
  ...
  data() {
    return {
      ...
      url: `http://127.0.0.1:8000/api/products/export`,
      lang: "ar",
      columns: [
        {
          label: this.$t("id"),
  ...
  methods: {
    async getResults() {
      ...
      const products = await this.$store.dispatch("fetchProducts", productsUrl);
      this.rows = this.$store.getters.getProducts.data;
    },
    async Delete(id) {
      ...
      if (result.value) {
        ...
        await this.$store.dispatch("deleteProducts", URL);
