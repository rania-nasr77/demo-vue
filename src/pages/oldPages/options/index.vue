<template>
  <div>
    <top-bar-card :newUrlName="`add-options`" />
...
export default {
  ...
  data() {
    return {
      ...
      url: `http://127.0.0.1:8000/api/options/export`,
      lang: "ar",
      columns: [
        {
          label: this.$t("id"),
  ...
  methods: {
    async getResults() {
      const optionUrl = { URL: "options", all: 1 };
      const options = await this.$store.dispatch("fetchOptions", optionUrl);
      this.rows = this.$store.getters.getOptions.data;
    },
    async Delete(id) {
      try {
        ...
        if (result.value) {
          ...
          await this.$store.dispatch("deleteOption", URL);
          this.rows = this.rows.filter((option) => {
            return option.id != id;
          });
        }
      } catch (errors) {
