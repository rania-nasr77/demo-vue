<template>
  <div>
    <top-bar-card :newUrlName="`add-branches`" />
...
export default {
  ...
  data() {
    return {
      ...
      url: `http://127.0.0.1:8000/api/branches/export`,
      lang: "ar",
      columns: [
        {
          label: this.$t("id"),
  ...
  methods: {
    async getResults() {
      const branchUrl = { URL: "branches", all: 1 };
      const branches = await this.$store.dispatch("fetchBranches", branchUrl);
      this.rows = this.$store.getters.getBranches.data;
    },
    ...
    async Delete(id) {
      ...
      if (result.value) {
        ...
        await this.$store.dispatch("deleteBranch", URL);
